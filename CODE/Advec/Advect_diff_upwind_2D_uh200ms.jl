...
function sub_advec_diff(GRD,Bio_in,K,U,V,ni,nj,dep)
	...
	for time=1:nt
	  ...
	  wrk1, wrk2 = horz_advect_diff_upwind(K,U,V,Tfield[:,:,t],ni,nj,dep)
...
function horz_advect_diff_upwind(K,uvel,vvel,Tracer,ni,nj,dep)
	isd = 1
	jsd = 2 #ignore j=1 b/c land (Antarctica)
	ied = ni
	jed = nj
	...
	for j=jsd:jed
		for i=isd:ied #i=isd-1:ied
			...
			if (GRD["lmask"][i,j,1] > 0)
				if (i == ied)
					afe[i,j]  = GRD["dyte"][i,j].*(upos.*Tracer[i,j]./dep[i,j] + uneg.*Tracer[isd,j]./dep[isd,j]) .*GRD["lmask"][i,j,1] .*GRD["lmask"][isd,j,1]
					dfe[i,j]  = GRD["dyte"][i,j].*(kpos.*gradT[i,j] + kneg.*gradT[isd,j]) .*GRD["lmask"][i,j,1] .*GRD["lmask"][isd,j,1]
				else
					afe[i,j]  = GRD["dyte"][i,j].*(upos.*Tracer[i,j]./dep[i,j] + uneg.*Tracer[i+1,j]./dep[i+1,j]) .*GRD["lmask"][i,j,1] .*GRD["lmask"][i+1,j,1]
	...
	for j=jsd:jed #j=jsd-1:jed
		for i=isd:ied
			...
			if (GRD["lmask"][i,j,1] > 0)
				if (j < jed)
					afn[i,j]  = GRD["dxtn"][i,j].*(upos.*Tracer[i,j]./dep[i,j] + uneg.*Tracer[i,j+1]./dep[i,j+1]) .*GRD["lmask"][i,j,1] .*GRD["lmask"][i,j+1,1]
					dfn[i,j]  = GRD["dxtn"][i,j].*(kpos.*gradT[i,j] + kneg.*gradT[i,j+1]) .*GRD["lmask"][i,j,1] .*GRD["lmask"][i,j+1,1]
				else
					afn[i,j]  = GRD["dxtn"][i,j].*(upos.*Tracer[i,j]./dep[i,j] + uneg.*Tracer[ni-i+1,j]./dep[ni-i+1,j]) .*GRD["lmask"][i,j,1] .*GRD["lmask"][ni-i+1,j,1]
