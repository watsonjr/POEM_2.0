program reduced_advect_upwind_nofn
  !
...
type :: ocean_adv_vel_type
  real, dimension(ni,nj,nk)   :: uhrho_et  !rho_dzu * advect vel (kg/(m*s)) on i-face of T-cell
  real, dimension(ni,nj,nk)   :: vhrho_nt  !rho*dzu * advect vel (kg/(m*s)) on j-face of T-cell
  real, dimension(ni,nj,0:nk) :: wrho_bt   !rho * vertical advect vel (kg/(m^2*s)) on T-bottom
...
real,dimension(ni,nj,nk)      :: tempk, templ
real,dimension(ni,nj,0:nk)    :: tempm
integer                       :: i, j, k
...
call random_number(tempm)

! Grid
Grid%ni = ni
Grid%nj = nj
Grid%nk = nk
Grid%dxtn = 1.1119e5 * temp
Grid%dyte = 1.1122e5 * temp
Grid%dat = 1.0870e10 * temp
...
call horz_advect_tracer(Adv_vel, T_prog, Tracer, ntracer, dtime, store_flux)
call vert_advect_tracer(Adv_vel, T_prog, Tracer, ntracer, dtime)
...
! </DESCRIPTION>
!
  subroutine horz_advect_tracer(Adv_vel, T_prog, Tracer, ntracer, dtime, store_flux)
        ...
        do k=1,nk
          ...
          do j=jsd,jed
            do i=isd-1,ied
              ...
              fe(i,j)  = Grd%dyte(i,j)*(upos*Tracer%field(i,j,k) + uneg*Tracer%field(i+1,j,k)) &
          ...
          do j=jsd-1,jed
            do i=isd,ied
              ...
              upos     = velocity + abs(velocity)
              uneg     = velocity - abs(velocity)
              fn(i,j)  = Grd%dxtn(i,j)*(upos*Tracer%field(i,j,k) + uneg*Tracer%field(i,j+1,k)) &
...
! </DESCRIPTION>
!
  subroutine vert_advect_tracer(Adv_vel, T_prog, Tracer, ntracer, dtime)
    ...
    if(.not. advect_sweby_all) then
      ...
      case (ADVECT_UPWIND)
        !Tracer%wrk1(isd:ied,jsd:jed,:) = &
        !-vert_advect_tracer_upwind(Adv_vel, Tracer%field(:,:,:))
...
end program reduced_advect_upwind_nofn
