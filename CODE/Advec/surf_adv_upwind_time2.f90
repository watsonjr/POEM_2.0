...
real,dimension(ni,nj,nt)      :: temp
real,dimension(ni,nj)         :: tempk, templ, tempj, tempi
integer                       :: i, j, k
...
! T_prog
T_prog%name = "PL"
T_prog%horz_advect_scheme = ADVECT_UPWIND
T_prog%vert_advect_scheme = ADVECT_UPWIND
...
do time=1,12
  ...
  ! Adv_vel
  Adv_vel%uhrho_et = 5.0 * templ
  Adv_vel%vhrho_nt = 5.0 * tempj

  ! Will need T_progr,ntracer if pass in all fish at once for looping over each
  !call horz_advect_tracer(time, Adv_vel, T_prog, Tracer, ntracer, store_flux)
  call horz_advect_tracer(time, Adv_vel, Tracer, store_flux)

  !write(*,*) 'Tracer%field : ', Tracer%field(1,1,1)
  !write(*,*) 'tendency : ', Tracer%tendency(1,1,1)
  !write(*,*) 'dt : ', dtime
  do j=jsd,jed
    do i=isd,ied
      Tracer%field(i,j,time+1) =  (Tracer%field(i,j,time) + dtime*Tracer%tendency(i,j,time))
    enddo
  enddo

enddo

write(*,*) 'Tracer%field : ', Tracer%field(1,1,:)
  ...
  subroutine horz_advect_tracer(time, Adv_vel, Tracer, store_flux)
    ...
    if(.not. advect_sweby_all) then
      ...
      case (ADVECT_UPWIND)
        ...
        -horz_advect_tracer_upwind(Adv_vel%uhrho_et, Adv_vel%vhrho_nt, Tracer%field(:,:,t))
      case default
...
end program surf_adv_upwind_time
