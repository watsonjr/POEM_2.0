...
type biom
	PISC::Array{Float64} # piscivore
	PLAN::Array{Float64} # planktivore
	DETR::Array{Float64} # detritivore
	W::Array{Float64} # detritus
end
...
function make_parameters()
	...
	Z_s = [128e-6; 70000e-6]; # note: max zoo size
	...
	function fnc_pij(w1,w2,beta) # Blanchard function to calculate diet preference
	...
	for i = 1:PI_N
		pij_pi = fnc_pij(PI_s[i],PI_s,2) # prey preference for PI
		pij_pl = fnc_pij(PI_s[i],PL_s,2) # prey preference for PL
		pij_de = fnc_pij(PI_s[i],DE_s,0) # prey preference for DE
		pij_z  = fnc_pij(PI_s[i],Z_s,2) # prey preference for DE
	...
	for i = 1:DE_N
		pij_de = fnc_pij(DE_s[i],DE_s,2) # prey preference for DE
