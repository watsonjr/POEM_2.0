...
function sub_init_local(PRM_PI,PRM_PL,PRM_DE)
	...
	# assign to types
	PISC = fish(bio_pi,met_pi,I_pi,tau_pi,nu_pi,gamma_pi,d_pi,REP_PI,GRW_PI,MAT_PI,enc_pi,ENC_pi)
	PLAN = fish(bio_pl,met_pl,I_pl,tau_pl,nu_pl,gamma_pl,d_pl,REP_PL,GRW_PL,MAT_PL,enc_pl,ENC_pl)
	DETR = fish(bio_de,met_de,I_de,tau_de,nu_de,gamma_de,d_de,REP_DE,GRW_DE,MAT_DE,enc_de,ENC_de)

	###! Detritus
	W = detritus(ones(Float64,1),zeros(Float64,1),zeros(Float64,1))
