...
function fnc_met!(meta::Array{Float64},i::Int,s::Float64,Temp::Float32)
	U = (3.9*s.^0.13)# swimming speed ind of temp
    activity = exp(0.03*U) # activity costs
    temp = exp(0.0548*Temp) # temperature costs 
...
function fnc_enc!(enc::Array{Float64,1},i::Int,prey::Array{Float64},
				  phi::Array{Float64},a::Float64)
	for j = 1:length(prey)
		enc[j,i] = prey[j].*phi[j].*a
	end
end
