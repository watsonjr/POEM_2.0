...
###! Metabolism
function sub_metabolism!(FISH::fish,X::Int,PRM::param,TEMP_p::Array{Float32})
	for i = 1:PRM.N
		FISH.met[X,i] = (0.0033*PRM.s[i]^-0.13) * (exp(0.0548*TEMP_p[X])) * 5.258
	end
end
...
###! Handling times
function sub_tau!(FISH::fish,X::Int,PRM::param)
	for i = 1:PRM.N
		FISH.tau[X,i] = 1. / (4. * FISH.met[X,i])
	end
end


###! Encounter rates
function sub_enc_pipi!(PISC::fish,X::Int,PRM_PI::param)
	for i = 1:PRM.N
		for j = 1:PRM.N
			PISC.enc["pipi"][X][j,i] = PISC.bio[X,j] * PRM_PI.Phi_PI[j,i] * PRM_PI.a[i]
		end
	end
end
function sub_enc_pipl!(PISC::fish,PLAN::fish,X::Int,PRM_PI::param,PRM_PL::param)
	for i = 1:PRM_PI.N
		for j = 1:PRM_PL.N
			PISC.enc["pipl"][X][j,i] = PLAN.bio[X,j] * PRM_PI.Phi_PL[j,i] * PRM_PI.a[i]
		end
	end
end
