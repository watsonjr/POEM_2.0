...
MASK = ncread([Cdir 'wc15n_avgmo_1988.nc'],'MASK');
yrs= 1988:2010;
...
for y = 1:nyrs
    ...
    di = [Cdir '/daily3km/Data_3km_' num2str(yr)];
    load([di '.mat'], 'NEMURO');
...
%%
ptemp_mean_3km = double(nanmean(D_Tp,2));
btemp_mean_3km = double(nanmean(D_Tb,2));
det_mean_3km = double(nanmean(D_det,2));
mz_mean_3km = double(nanmean(D_Zm,2));
lz_mean_3km = double(nanmean(D_Zl,2));
mzloss_mean_3km = double(nanmean(D_dZm,2));
lzloss_mean_3km = double(nanmean(D_dZl,2));

%%
save([Cdir 'nemuro_3km_means.mat'],'WID','D_Tp','D_Tb','D_Zm','D_Zl','D_dZm',...
    'D_dZl','D_det','ptemp_mean_3km','btemp_mean_3km','det_mean_3km',...
    'mz_mean_3km','lz_mean_3km','mzloss_mean_3km','lzloss_mean_3km');
