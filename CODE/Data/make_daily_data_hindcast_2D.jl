...
lmask = load("./Data_grid_cp_2D.jld","lmask")
smask = lmask[:,:,1];
WID = find(lmask[:,:,1] .== 1.0); # spatial index of water cells
...
for t in [1:2]
  ...
  v = V[:,:,I];
	...
	D_Tp  = zeros(ni,nj,365);
	D_Tb  = zeros(ni,nj,365);
	D_Zm  = zeros(ni,nj,365);
 	D_Zl  = zeros(ni,nj,365);
	D_dZm = zeros(ni,nj,365);
 	D_dZl = zeros(ni,nj,365);
 	D_det = zeros(ni,nj,365);
  D_u = zeros(ni,nj,365);
  D_v = zeros(ni,nj,365);
  ...
  v[find(v.==minimum(v))] = 0.0
    ...
    for n = 1:nj
      if (smask[m,n] .== 1.0)
        ...
        #! v currents
    		...
    		Yi = yi[Xi[1:end-1]];
    		D_v[m,n,1:length(Yi)] = (Yi / 100 * 60 *60 * 24); # m d-1
    		...
    		Xi= [time[1]:1:time[end]];
    		Yi = yi[Xi[1:end-1]];
    		D_u[m,n,1:length(Yi)] = Yi / 100 * 60 *60 * 24; # m d-1
    		...
    		Xi= [time[1]:1:time[end]];
    		Yi = yi[Xi[1:end-1]];
    		D_Tb[m,n,1:length(Yi)] = Yi ## FIX THIS LATER FOR POT TEMP
    		...
    		Xi= [time[1]:1:time[end]];
            Yi = yi[Xi[1:end-1]];
    		D_Zm[m,n,1:length(Yi)] = Yi * (106/16) * 12.01 / 0.32;
    		...
    		Xi= [time[1]:1:time[end]];
            Yi = yi[Xi[1:end-1]];
    		D_Zl[m,n,1:length(Yi)] = Yi * (106/16) * 12.01 / 0.32;
    		...
    		Xi= [time[1]:1:time[end]];
    		Yi = yi[Xi[1:end-1]];
    		D_dZm[m,n,1:length(Yi)] = Yi * (106/16) * 12.01 / 0.32 * 60 * 60 *24 ;
    		...
    		Xi= [time[1]:1:time[end]];
            Yi = yi[Xi[1:end-1]];
    		D_dZl[m,n,1:length(Yi)] = Yi * (106/16) * 12.01 / 0.32 *60 *60 *24;
    		...
    		Xi= [time[1]:1:time[end]];
            Yi = yi[Xi[1:end-1]];
    		D_det[m,n,1:length(Yi)] = Yi * (106/16) * 12.01 / 0.32 *60 *60 *24;
      end #water cells
	 end #lat
 end #lon
	...
	#! save
	println(t)
	ti = string(1000000+t); di = "./JLD/Data_hindcast2D_";
