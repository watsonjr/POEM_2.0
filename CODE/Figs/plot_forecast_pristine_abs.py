...
bio_pi_early = np.median(PISC[0:12*10,:,:],0).T
bio_pi_late  = np.median(PISC[-(12*10):-1,:,:],0).T

#! Average over large size classes
bio_pi_early = np.median(bio_pi_early[:,5:-1],1)
bio_pi_late = np.median(bio_pi_late[:,5:-1],1)
...
plot_abs(Lon,Lat,bio_pi_late,ID)
...
##! Data to plot -- PLAN
#! Average over ten years
bio_pl_early = np.median(PLAN[0:12*10,:,:],0).T
bio_pl_late  = np.median(PLAN[-(12*10):-1,:,:],0).T

#! Average over large size classes
bio_pl_early = np.median(bio_pl_early[:,5:-1],1)
bio_pl_late = np.median(bio_pl_late[:,5:-1],1)
...
ax0 = plt.subplot(gs[1,:])
plot_abs(Lon,Lat,bio_pl_late,ID)
plt.tight_layout(h_pad=0.5)
...
##! Data to plot -- DETR
#! Average over ten years
bio_de_early = np.median(DETR[0:12*10,:,:],0).T
bio_de_late  = np.median(DETR[-(12*10):-1,:,:],0).T

#! Average over large size classes
bio_de_early = np.median(bio_de_early[:,5:-1],1)
bio_de_late = np.median(bio_de_late[:,5:-1],1)
