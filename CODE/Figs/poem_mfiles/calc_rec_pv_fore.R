...
# load data
f <- read.csv("onelocs_fore_forage_larv.csv",sep=",", header = T,
              stringsAsFactors = F)
d <- read.csv("onelocs_fore_dem_larv.csv",sep=",", header = T,
              stringsAsFactors = F)
p <- read.csv("onelocs_fore_pisc_larv.csv",sep=",", header = T,
              stringsAsFactors = F)
nid <- ncol(f)
...
# 2021-2050
Rstats <- as.data.frame(names(d)[2:13])
for(i in 1:nid-1){
  #Population variability metric of Heath 2006
  #PV of R
  
  #Convert from biomass to abundance
  dnum <- round(d[2:31,i+1] / 2529.822128134704 * A[i]) + 1
  pnum <- round(p[2:31,i+1] / 2529.822128134704 * A[i]) + 1
  fnum <- round(f[2:31,i+1] / 2.5298221281347053 * A[i]) + 1
...
write.table(Rstats,"onelocs_fore_2021-2050_recruitment_PV.csv",row.names=F,sep=",")

# 2071-2100
Rstats2 <- as.data.frame(names(d)[2:13])
for(i in 1:nid-1){
  #Population variability metric of Heath 2006
  #PV of R
  
  #Convert from biomass to abundance
  dnum <- round(d[52:81,i+1] / 2529.822128134704 * A[i]) + 1
  pnum <- round(p[52:81,i+1] / 2529.822128134704 * A[i]) + 1
  fnum <- round(f[52:81,i+1] / 2.5298221281347053 * A[i]) + 1
...
names(Rstats2) <- c("ID","F","P","D")
write.table(Rstats2,"onelocs_fore_2071-2100_recruitment_PV.csv",row.names=F,sep=",")
