    ...
    for n = 1:nt
        ...
        for j=jsd:jed
            for i=isd:ied
                if (j==nj)
                    if (i==1)
                        KK(i,j) = max([nu(i,j),nu(ni-i+1,j),nu(i,j-1),nu(ied,j),nu(i+1,j)]);
                    elseif (i==ni)
                        KK(i,j) = max([nu(i,j),nu(ni-i+1,j),nu(i,j-1),nu(i-1,j),nu(isd,j)]);
                    else
                        KK(i,j) = max([nu(i,j),nu(ni-i+1,j),nu(i,j-1),nu(i-1,j),nu(i+1,j)]);
                    end
                else
                    if (i==1)
                        KK(i,j) = max([nu(i,j),nu(i,j+1),nu(i,j-1),nu(ied,j),nu(i+1,j)]);
                    elseif (i==ni)
                        KK(i,j) = max([nu(i,j),nu(i,j+1),nu(i,j-1),nu(i-1,j),nu(isd,j)]);
                    else
                        KK(i,j) = max([nu(i,j),nu(i,j+1),nu(i,j-1),nu(i-1,j),nu(i+1,j)]);
                    end
                end
            end %i
        end %j
