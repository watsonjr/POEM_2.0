...
% sims = {'3.75e-3','3.5e-3','3.25e-3'};
% rfrac = {'00038','00035','00032'};
fcrit = 40;
nmort = 'M2';
kad = 100;
pref = 'D050';
...
for r = 1:length(rfrac)
    ...
    for i=1:length(frate)
        ...
        dp = ['Dc_TrefO_Hartvig_cmax-metab_MFeqMP_fcrit' num2str(fcrit) ...
            '_' pref '_nmort'  nmort '_BE05_RE' num2str(RE) '_LP_fish' F];
%         dp = ['Dc_TrefO_Hartvig_cmax-metab_MFeqMP_fcrit' num2str(fcrit) ...
%             '_MZ01_nmort'  nmort '_BE05_RE' num2str(RE) '_BAassim_LP_fish' F];
        dpath = [datap char(dp) '/'];
        fpath = [figp char(dp) '/'];
        cfile = char(dp);
        cfile2 = ['Dc_TrefO_Hartvig_cmax-metab_MFeqMP_fcrit' num2str(fcrit) ...
            '_' pref '_nmort'  nmort '_BE05_RE' num2str(RE) '_LP_fishing_catch'];
%         cfile2 = ['Dc_TrefO_Hartvig_cmax-metab_MFeqMP_fcrit' num2str(fcrit) ...
%             '_MZ01_nmort'  nmort '_BE05_RE' num2str(RE) '_BAassim_LP_fishing_catch'];
...
%                 '_MZ01_nmort'  nmort '_BE05_RE' num2str(RE) '_BAassim_LP_fish' F];
        ...
        print(f7,'-dpng',[figp loc '/' sname sname2 cfile2 '_' lname 'P.png'])
...
%                 '_MZ01_nmort'  nmort '_BE05_RE' num2str(RE) '_BAassim_LP_fish' F];
            ...
            f12 = figure(12);
            subplot(4,3,s)
            plot(i,Ptot,'.k','MarkerSize',25); hold on;
            xlim([0 length(frate)+1])
            if (i==length(frate))
                set(gca,'XTick',1:length(frate),'XTickLabel',fsim);
                if (s==2)
    ...
    print(f12,'-dpng',[figp sname sname2 cfile2 '_allP_locs.png'])
end
