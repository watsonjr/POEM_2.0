...
### ADVECTION ###
function sub_advec2Dfull(Bio_in,Nu,U,V,DX,DY,Tp,Tb,tdif,wgt)
	#Bio_in = bio; Nu = Med_f.nu; DX = GRD["dxtn"]; DY = GRD["dyte"]; U = ENVR.U; V = ENVR.V;
			...
			if (i==1)
				if (j==nj)
					BB = bio2[i,j] - ((UU/2)*(bio2[i+1,j]-bio2[360,j])) -
									...
									- ((VV/2)*(bio2[ni-i+1,j]-bio2[i,j-1])) -
									(((VV^2)/2)*(bio2[ni-i+1,j]-(2*bio2[i,j])+bio2[i,j-1]))
									- (UU*VV/4)*(bio2[ni-(i+1)+1,j]-bio2[ni-360+1,j]-bio2[i+1,j-1]-bio2[360,j-1])
				else
					BB = bio2[i,j] - ((UU/2)*(bio2[i+1,j]-bio2[360,j])) -
								(((UU^2)/2)*(bio2[i+1,j]-(2*bio2[i,j])+bio2[360,j]))
								- ((VV/2)*(bio2[i,j+1]-bio2[i,j-1])) -
								(((VV^2)/2)*(bio2[i,j+1]-(2*bio2[i,j])+bio2[i,j-1]))
								- (UU*VV/4)*(bio2[i+1,j+1]-bio2[360,j+1]-bio2[i+1,j-1]-bio2[360,j-1])
				end
			elseif (i==ni)
				if (j==nj)
					BB = bio2[i,j] - ((UU/2)*(bio2[1,j]-bio2[i-1,j])) -
								...
								- ((VV/2)*(bio2[ni-i+1,j]-bio2[i,j-1])) -
								(((VV^2)/2)*(bio2[ni-i+1,j]-(2*bio2[i,j])+bio2[i,j-1]))
								- (UU*VV/4)*(bio2[ni-1+1,j]-bio2[ni-(i-1)+1,j]-bio2[1,j-1]-bio2[i-1,j-1])
				else
					BB = bio2[i,j] - ((UU/2)*(bio2[1,j]-bio2[i-1,j])) -
								(((UU^2)/2)*(bio2[1,j]-(2*bio2[i,j])+bio2[i-1,j]))
								- ((VV/2)*(bio2[i,j+1]-bio2[i,j-1])) -
								(((VV^2)/2)*(bio2[i,j+1]-(2*bio2[i,j])+bio2[i,j-1]))
								- (UU*VV/4)*(bio2[1,j+1]-bio2[i-1,j+1]-bio2[1,j-1]-bio2[i-1,j-1])
				end
			else
				if (j==nj)
					BB = bio2[i,j] - ((UU/2)*(bio2[i+1,j]-bio2[i-1,j])) -
								...
								- ((VV/2)*(bio2[ni-i+1,j]-bio2[i,j-1])) -
								(((VV^2)/2)*(bio2[ni-i+1,j]-(2*bio2[i,j])+bio2[i,j-1]))
								- (UU*VV/4)*(bio2[ni-(i+1)+1,j]-bio2[ni-(i-1)+1,j]-bio2[i+1,j-1]-bio2[i-1,j-1])
				else
					BB = bio2[i,j] - ((UU/2)*(bio2[i+1,j]-bio2[i-1,j])) -
								(((UU^2)/2)*(bio2[i+1,j]-(2*bio2[i,j])+bio2[i-1,j]))
								- ((VV/2)*(bio2[i,j+1]-bio2[i,j-1])) -
								(((VV^2)/2)*(bio2[i,j+1]-(2*bio2[i,j])+bio2[i,j-1]))
								- (UU*VV/4)*(bio2[i+1,j+1]-bio2[i-1,j+1]-bio2[i+1,j-1]-bio2[i-1,j-1])
				end
			end
			if BB < 0
				Bio_minus[i,j] = 0
