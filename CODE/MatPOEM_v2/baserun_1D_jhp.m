...
for L = 1:length(ids);
    ...
    for YR = 1:param.tEnd % years
        for DAY = 1:365
            ...
            if (YR==1 && DAY==1)
                result = poem_1D_jhp(param,ENVR,t);
            else
                result = poem_1D_jhp(param,ENVR,t,result);
            end
        end
    end
    save(['/Volumes/GFDL/CSV/Matlab_new_size/',simname,'/baserun_jhp_log_' loc '.mat'],'result')
    %! Plot
    plotPoem_1D(param, result)
    print('-dpng',['/Users/cpetrik/Dropbox/Princeton/POEM_2.0/CODE/Figs/PNG/Matlab_New_sizes/baserun_jhp_log_' simname '_' loc])
