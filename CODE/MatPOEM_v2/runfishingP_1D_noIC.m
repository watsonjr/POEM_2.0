...
for L = 6:length(ids);
    ...
    for i = 1:length(F)
        %Fish P
        frate = F(i);
        param.F = param.dt * [0 0   0 0.1*F(i) F(i)     0 0 0];
        t=0;
        ...
        %! Save
        dirname
        save([dpath,simname,'/fishing_log_noIC_' harv tfish(2:end) '_' loc '.mat'],'result')
        %! Plot
        plotPoem_1D(param, result)
        print('-dpng',[fpath,simname,'/fishing_log_noIC_' harv tfish(2:end) '_' loc])
        clear result
    end
    %! Save
    dirname
    save([dpath,simname,'/fishing_log_noIC_' harv '_' loc '.mat'],'Y','F')
    ...
    print('-dpng',[fpath,simname,'/fishing_log_noIC_' harv '_' simname '_' loc])
