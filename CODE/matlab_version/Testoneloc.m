...
for F = 1:length(Fmort)
    ...
    if (harv==1)
        %simname = ['Dc_TrefO_Hartvig_cmax-metab_MFeqMP_fcrit',tfcrit,'_BAassim','_nmort',tmort,'_BE',tbe(2:end),'_RE',tre(2:end),'_',sel,'_fish',tfish(2:end)];
        simname = ['Dc_TrefO_Hartvig_cmax-metab_MFeqMP_fcrit',tfcrit,'_D',tld(2:end),'_nmort',tmort,'_BE',tbe(2:end),'_RE',tre(2:end),'_',sel,'_fish',tfish(2:end)];
    else
        %simname = ['Dc_TrefO_Hartvig_cmax-metab_MFeqMP_fcrit',tfcrit,'_BAassim','_nmort',tmort,'_BE',tbe(2:end),'_RE',tre(2:end)];
        simname = ['Dc_TrefO_Hartvig_cmax-metab_MFeqMP_fcrit',tfcrit,'_D',tld(2:end),'_nmort',tmort,'_BE',tbe(2:end),'_RE',tre(2:end)];
    end
...
%     end
    ...
    for L = 1;%:11
        ...
        %! Storage
        Spinup_Sml_f = NaN*ones(DAYS,28);
        Spinup_Sml_p = NaN*ones(DAYS,28);
        Spinup_Sml_d = NaN*ones(DAYS,28);
        Spinup_Med_f = NaN*ones(DAYS,28);
        Spinup_Med_p = NaN*ones(DAYS,28);
        Spinup_Med_d = NaN*ones(DAYS,28);
        Spinup_Lrg_p = NaN*ones(DAYS,28);
        Spinup_Lrg_d = NaN*ones(DAYS,28);
        Spinup_Cobalt = NaN*ones(DAYS,4);
