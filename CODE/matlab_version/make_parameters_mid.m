...
function make_parameters_mid()
    ...
    amet = 4;       % coeff on met
    h = 50;         % coeff on Cmax
    gam = 50;       % coeff on search area
    kc = 0.0604;    % coeff on cmax T-dep fn (q10=1.83)
    ke = 0.0604;    % coeff on enc T-dep fn 
    kt = 0.0604;    % coeff on met T-dep fn (orig 0.063)
    bpow = 0.21;    % power on metab fn (orig 0.25)
    benc = 0.21;    % power on enc fn (orig 0.20)
    bcmx = 0.21;    % power on cmax fn (orig 0.25)
