...
rall = NaN*ones(length(kays),length(Fish));
rF = rall;
rP = rall;
rD = rall;
rmse = rall;
...
for b=1:length(Fish)
    for k = 1:6;%length(kays)
        ...
        text(-3.5,1.5,['r = ' num2str(rall(k,b))])
        text(-3.5,1.0,['RMSE = ' num2str(rmse(k,b))])
        axis([-4 2 -4 2])
        xlabel('SAUP mean of top 10 years')
        ...
        text(-5.5,1.5,['r = ' num2str(rF(k,b))])
        text(-5.5,1.0,['RMSE = ' num2str(rmseF(k,b))])
        axis([-6 2 -6 2])
        xlabel('SAUP F catch (log10 MT km^-^2)')
        ylabel('POEM F catch (log10 MT km^-^2)')
        title('Mean F catch')
        ...
        text(-5.5,1.5,['r = ' num2str(rP(k,b))])
        text(-5.5,1.0,['RMSE = ' num2str(rmseP(k,b))])
        axis([-6 2 -6 2])
        xlabel('SAUP P catch (log10 MT km^-^2)')
        ylabel('POEM P catch (log10 MT km^-^2)')
        title('Mean P catch')
        ...
        text(-5.5,1.5,['r = ' num2str(rD(k,b))])
        text(-5.5,1.0,['RMSE = ' num2str(rmseD(k,b))])
        axis([-6 2 -6 2])
        xlabel('SAUP D catch (log10 MT km^-^2)')
        ylabel('POEM D catch (log10 MT km^-^2)')
        title('Mean D catch')
...
save([dp 'Clim_b',tbfn(2:end),'_fished',harv,'_LME_SAUP_catch_comp_F_k.mat'],'rall','rF','rP',...
    'rD','rmse','rmseF','rmseP','rmseD')

%% Plots of r and RMSE
cfile2 = ['Clim_fished',harv,'_Dc_enc70_cmax-metab20_b',tbfn(2:end),'_fcrit20_D075_J100_A050_Sm025_nmort1_BE05_CC050_lgRE00100_mdRE00100'];

kays = 0.0405:0.01:0.1;
...
Rall(1:nc,1:ne)=rall(1:nc,1:ne);
RF(1:nc,1:ne)=rF(1:nc,1:ne);
RP(1:nc,1:ne)=rP(1:nc,1:ne);
RD(1:nc,1:ne)=rD(1:nc,1:ne);
...
colorbar
caxis([0.2 0.4])
set(gca,'XTickLabel',Fish(1:2:end),'YTickLabel',kays)
xlabel('Fishing rate')
...
title('Corr D')
stamp(cfile2)
print('-dpng',[pp 'Clim_b',tbfn(2:end),'_fished',harv,'_LME_SAUP_catch_corr_F_k'])
...
Rmseall(1:nc,1:ne)=rmse(1:nc,1:ne);
RmseF(1:nc,1:ne)=rmseF(1:nc,1:ne);
RmseP(1:nc,1:ne)=rmseP(1:nc,1:ne);
RmseD(1:nc,1:ne)=rmseD(1:nc,1:ne);
...
colorbar
caxis([0.5 0.8])
set(gca,'XTickLabel',Fish(1:2:end),'YTickLabel',kays(2:2:end))
xlabel('Fishing rate')
ylabel('Temp exponent')
title('RMSE all fish')
...
title('RMSE D')
stamp(cfile2)
print('-dpng',[pp 'Clim_b',tbfn(2:end),'_fished',harv,'_LME_SAUP_catch_rmse_F_k'])
