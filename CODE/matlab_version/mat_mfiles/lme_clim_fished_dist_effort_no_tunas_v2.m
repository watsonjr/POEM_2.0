...
for k=1:length(cuts)
    ...
    maxd = max(min_dist1(:));
    mind = min(min_dist1(:));
    eff1 = mins(q) + (maxs(q)-mins(q)).*(min_dist1-mind) ./ (maxd-mind);
    
    min_dist2 = -1*min_dist.^(1/2);
    maxd = max(min_dist2(:));
    mind = min(min_dist2(:));
    eff2 = mins(q) + (maxs(q)-mins(q)).*(min_dist2-mind) ./ (maxd-mind);
    
    min_dist3 = -1*min_dist.^(1/3);
    maxd = max(min_dist3(:));
    mind = min(min_dist3(:));
    eff3 = mins(q) + (maxs(q)-mins(q)).*(min_dist3-mind) ./ (maxd-mind);
    
    min_distL = 1 ./ (1 + exp(-(10/cutkm)*(min_dist-cutkm)));
    maxd = max(min_distL(:));
    mind = min(min_distL(:));
    effL = mins(q) + (maxs(q)-mins(q)).*(min_distL-mind) ./ (maxd-mind);
    effL = 1-effL;
    
    min_distS = 1+tanh((10/cutkm)*(min_dist-cutkm));
    maxd = max(min_distS(:));
    mind = min(min_distS(:));
    effS = mins(q) + (maxs(q)-mins(q)).*(min_distS-mind) ./ (maxd-mind);
    effS = 1-effS;
    
    min_distG = exp(-cutkm.*exp((-10/cutkm)*min_dist));
    maxd = max(min_distG(:));
    mind = min(min_distG(:));
    effG = mins(q) + (maxs(q)-mins(q)).*(min_distG-mind) ./ (maxd-mind);
    effG = 1-effG;
    
    %Pick effort shape
    eff=effG;
    eff(min_dist>cutkm) = 0;
    cfile2 = ['_effort_gomp_',num2str(cutkm),'km_no_tunas_bathy'];
    tit2 = ['Gompertz Effort ',num2str(cutkm),'km'];
