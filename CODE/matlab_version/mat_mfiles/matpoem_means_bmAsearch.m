...
SF = NaN*ones(length(bmp),length(aparam),16);
SP = SF;
SD = SF;
MF = SF;
MP = SF;
...
for j=1:length(bmp)
    ...
    sfile = ['/Volumes/GFDL/NC/Matlab_new_size/',cfile,...
        '/param_sens/Climatol_All_fish03_m-b',tbfn(2:end),'_A',ta(2:end),'_locs.mat'];
    load(sfile);
...
save([nfile 'Locs_Climatol_All_fish03_means_bmAsearch.mat'],'SF','SP','SD',...
