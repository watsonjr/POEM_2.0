...
SF = NaN*ones(length(bees),length(ktemp),16);
SP = SF;
SD = SF;
MF = SF;
MP = SF;
...
for j=1:length(bees)
    ...
    sfile = ['/Volumes/GFDL/NC/Matlab_new_size/',cfile,...
        '/param_sens/Climatol_All_fish03_m-k',tkfn(2:end),'_BE',tbe(2:end),'_locs.mat'];
    load(sfile);
...
save([nfile 'Locs_Climatol_All_fish03_means_kt_BE_search.mat'],'SF','SP','SD',...
