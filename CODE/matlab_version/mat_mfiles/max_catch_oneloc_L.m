...
cfile2 = ['Dc_TrefO_cmax-metab2_enc1_MFeqMP_fcrit' num2str(fcrit) ...
            '_' pref '_nmort' num2str(2) '_BE' BE '_CC' CC '_L_fishing_catch'];
...
for n = 2;%1:length(nmrt)
    ...
    for r = 1:length(RE)
        ...
        for i=1:length(frate)
            ...
            dpath = [char(dp) '/'];
            load([datap dpath sname 'lastyr_sum_mean_biom'],'Dtotcatch',...
                'Ptotcatch','all_mean');
...
cfile4 = ['Dc_TrefO_Hartvig_cmax-metab_MFeqMP_fcrit' num2str(fcrit) ...
                '_' pref '_BE' BE '_CC' CC '_L_BmsyB0'];
%%
...
for s = 1:length(spots)
    ...
    if (s==4)
        ylabel('L Bmsy:B0 in final year')
    end
end
stamp(cfile2)
...
%     'B0');
save([figp cfile2 '_allRE_allMort.mat'],'Cmax','m2_maxC','m2_BF','B0');
