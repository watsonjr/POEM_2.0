...
for s=1:length(spots)
    ...
    f10 = figure(10);
    subplot(4,3,s)
    plot(fnum,squeeze(caught(1,s,:)),'.k','MarkerSize',25); hold on;
    xlim([0 fnum(end)+0.1])
    title(loc)
    if (s==4)
        ylabel([Mname ' catch (g) in final year'])
    end
    if (s==11)
        xlabel('Annual fishing rate')
    end
    ...
    f11 = figure(11);
    subplot(4,3,s)
    plot(fnum,squeeze(caught(2,s,:)),'.k','MarkerSize',25); hold on;
    xlim([0 fnum(end)+0.1])
    title(loc)
    if (s==4)
        ylabel([Lname ' catch (g) in final year'])
    end
    if (s==11)
        xlabel('Annual fishing rate')
    end
    ...
    title(loc)
    if (s==4)
        ylabel(['Total ' sel ' catch (g) in final year'])
    end
    if (s==11)
        xlabel('Annual fishing rate')
    end
