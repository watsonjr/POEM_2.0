...
for j = 1:length(jays)
    ...
    dp = ['Dc_enc70-b200_cm20_m-b175-k09_fcrit20_c-b250_D075_',...
        'J100_A050_Sm025_nmort1_BE05_noCC_RE00100'];
    dpath = [datap char(dp) '/'];
    fpath = [figp char(dp) '/'];
    cfile = char(dp);
    ...
    load([dpath sname '_Juve' tJ(2:end) '.mat'])
    SP = S_Sml_p;
    SF = S_Sml_f;
    SD = S_Sml_d;
    MP = S_Med_p;
    ...
    save([dpath sname '_Juve' tJ(2:end) '_lastyr_sum_mean_biom.mat'],'Pmean','Fmean','Dmean','all_mean',...
...
yield  = NaN*ones(5,np,length(spots));
mcon  = NaN*ones(3,np);
mlev  = NaN*ones(3,np);
for n = 1:length(jays)
    Jsel = jays(n);
    ...
    fpath = [figp char(dp) '/'];
    cfile = char(dp);
    cfile2 = [dp '_JharvTests'];
    
    load([dpath sname '_Juve' tJ(2:end) '_lastyr_sum_mean_biom.mat']);
    
    %% Biomass of each type
    fishsp(:,:,n) = squeeze(nansum(all_mean));
    
    %% Catch of each type
    yield(1,n,:) = Ftotcatch(2,:);
    yield(2,n,:) = Ptotcatch(2,:);
    yield(3,n,:) = Dtotcatch(2,:);
    yield(4,n,:) = Ptotcatch(3,:);
    yield(5,n,:) = Dtotcatch(3,:);
