...
%% Only use 3 domain examples: EBS(10), PUp(16), HOT(13)
sid = [10;16;13];
rbcp = -0.325:0.025:-0.1;
rbmp = -0.325:0.025:-0.1;
for s=1:length(sid)
    domain = sid(s);
    loc = spots{domain};
    lname = [loc '_'];
    ...
    colormap(cmBP)
    caxis([-2 2])
    set(gca,'XTick',rbcp(1:2:end)-0.0125,'XTickLabel',{'-.3','-.25','-.2','-.15','-.1'},...
        'YTick',rbmp(1:2:end)-0.0125,'YTickLabel',rbmp(1:2:end))
    if (s==2)
...
%%
print(f2,'-dpng',[figp pname '_mean_biomass_type_all_3locs_v2.png'])
print(f5,'-dpng',[figp pname '_frac_all_3locs_v2.png'])
