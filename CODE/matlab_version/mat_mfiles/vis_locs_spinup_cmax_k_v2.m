...
for n = 1:length(kays)
    ...
    fpath = [figp char(dp) '/'];
    cfile = char(dp);
    cfile2 = ['Dc_enc',tefn,'_cmax-metab',tcfn,'_b',tbfn(2:end),'_fcrit',num2str(fcrit),'_',...
            D,'_',J,'_',Ad,'_',Sm,'_nmort',nmort,'_BE',BE,'_CC',CC,...
            '_lgRE',rfrac,'_mdRE',rfrac2,'_metKtests'];
...
subplot(3,1,1)
bar(mcon(1,:))
set(gca,'XTick',1:np,'XTickLabel',kays); hold on;
...
bar(mlev(1,:))
ylim([0 1])
set(gca,'XTick',1:np,'XTickLabel',kays,'YTick',0.2:0.2:0.8)
ylabel('S')
