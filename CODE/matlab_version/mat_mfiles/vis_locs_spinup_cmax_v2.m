...
for n = 1:length(cmaxs)
    ...
    fpath = [figp char(dp) '/'];
    cfile = char(dp);
    cfile2 = ['Dc_enc',tefn,'_fcrit',num2str(fcrit),'_',...
            D,'_',J,'_',Ad,'_',Sm,'_nmort',nmort,'_BE',BE,'_CC',CC,...
            '_lgRE',rfrac,'_mdRE',rfrac2,'_Cmaxtests'];
...
subplot(3,1,1)
bar(mcon(1,:))
set(gca,'XTick',1:np,'XTickLabel',cmaxs); hold on;
...
bar(mlev(1,:))
ylim([0 1])
set(gca,'XTick',1:np,'XTickLabel',cmaxs,'YTick',0.2:0.2:0.8)
ylabel('S')
