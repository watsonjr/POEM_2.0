...
for C = 1:length(cof)
    cff = cof(C);
    dp = ['Dc_enc',tefn,'_cmax-metab',tcfn,'_n',num2str(cff),'_fcrit',num2str(fcrit),'_',...
            D,'_',J,'_',Ad,'_',Sm,...
...
for n = 1:length(cof)
    ...
    fpath = [figp char(dp) '/'];
    cfile = char(dp);
    cfile2 = ['Dc_enc',tefn,'_cmax-metab',tcfn,'_fcrit',num2str(fcrit),'_',...
            D,'_',J,'_',Ad,'_',Sm,'_nmort',nmort,'_BE',BE,'_CC',CC,...
            '_lgRE',rfrac,'_mdRE',rfrac2,'_CmaxCoefftests'];
...
% Save values for all locs and all cof that combo
...
for s=1:length(spots)
    ...
    for t=1:np
        text(t,-2.1,num2str(cof(t)),'Rotation',45,'HorizontalAlignment','right')
    end
    if (s==11)
        text(13,1,['D=' num2str(D)]);
...
% subplot(3,1,1)
% bar(mcon(1,:))
% set(gca,'XTick',1:np,'XTickLabel',cof); hold on;
...
bar(mlev(1,:))
ylim([0 1])
set(gca,'XTick',1:np,'XTickLabel',cof,'YTick',0.2:0.2:0.8)
ylabel('S')
