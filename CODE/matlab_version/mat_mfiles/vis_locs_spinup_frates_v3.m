...
%     
%     load([dpath sname 'locs_fish_',harv,'.mat'])
%     SP = S_Sml_p;
%     SF = S_Sml_f;
%     SD = S_Sml_d;
%     MP = S_Med_p;
...
%     
%     save([dpath sname,'locs_fish_',harv,'_lastyr_sum_mean_biom.mat'],'Pmean','Fmean','Dmean','all_mean',...
...
mcon  = NaN*ones(3,np);
mlev  = NaN*ones(3,np);
meanf = NaN*ones(3,length(spots),np);
MFsel = 0.5;
% LPsel = bees(n);
LDsel = 3;
for n = 1:length(bees)
...
%     LDsel = bees(n);
    ...
    fpath = [figp char(dp) '/'];
    cfile = char(dp);
    cfile2 = ['Dc_enc70-b200_cm20_m-b175-k09_fcrit20_c-b250_D075_J100',...
        '_A050_Sm025_nmort1_BE05_CC100_lgRE00100_mdRE00100_TqFsel_tests'];
    
    load([dpath sname,'locs_Tfish_',harv,'_lastyr_sum_mean_biom.mat']);
...
for s=1:length(spots)
    ...
    f4=figure(4);
    subplot(4,4,s)
    plot(1-0.25:np,(squeeze(meanf(1,s,:)))*365,'sk','MarkerFaceColor',cmap_ppt(3,:),...
        'MarkerSize',15); hold on;
    plot(1:np,(squeeze(meanf(2,s,:)))*365,'sk','MarkerFaceColor',cmap_ppt(1,:),...
        'MarkerSize',15); hold on;
    plot(1+0.25:np+1,(squeeze(meanf(3,s,:)))*365,'sk','MarkerFaceColor',cmap_ppt(2,:),...
        'MarkerSize',15); hold on;
    xlim([0 np+1])
    ylim([0 1])
    if (s==5)
        ylabel('Mean fishing rate (yr^-^1)')
    end
    set(gca,'XTick',1:np,'XTickLabel',[]);
    for t=1:np
        text(t,-0.1,num2str(bees(t)),'Rotation',45,'HorizontalAlignment','right')
    end
    stamp(cfile2)
    title(loc)
...
print(f4,'-dpng',[figp sname cfile2 '_mean_frate_type_all_locs.png'])

Fmeanf = squeeze(meanf(1,:,:))*365;
Pmeanf = squeeze(meanf(2,:,:))*365;
Dmeanf = squeeze(meanf(3,:,:))*365;

cF = cell(size(Fmeanf)+1);
cF(2:end,2:end)=num2cell(Fmeanf);
vars=num2str([frate*bees]);
cF(1,2:end)=cellstr(vars)';
cF(2:end,1)=spots;
cF{1,1}='Location';
TF = cell2table(cF);

cP = cell(size(Pmeanf)+1);
cP(2:end,2:end)=num2cell(Pmeanf);
vars=num2str([frate*bees]);
cP(1,2:end)=cellstr(vars)';
cP(2:end,1)=spots;
cP{1,1}='Location';
TP = cell2table(cP);

cD = cell(size(Dmeanf)+1);
cD(2:end,2:end)=num2cell(Dmeanf);
vars=num2str([frate*bees]);
cD(1,2:end)=cellstr(vars)';
cD(2:end,1)=spots;
cD{1,1}='Location';
TD = cell2table(cD);
...
save([datap 'Bio_rates/' cfile2 '.mat'],'fishsp','mcon','mlev',...
    'TF','TP','TD');
writetable(TF,[datap 'Bio_rates/' cfile2 '_FmeanF.csv'],'Delimiter',',');
writetable(TP,[datap 'Bio_rates/' cfile2 '_FmeanP.csv'],'Delimiter',',');
writetable(TD,[datap 'Bio_rates/' cfile2 '_FmeanD.csv'],'Delimiter',',');
