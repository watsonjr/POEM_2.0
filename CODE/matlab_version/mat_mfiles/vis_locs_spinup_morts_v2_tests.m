...
%%
for n = 1:length(morts)
    nmort = num2str(morts(n));
...
for n = 1:np
    ...
    cfile2 = ['Dc_enc',tefn,'_cmax-metab',tcfn,'_fcrit',num2str(fcrit),'_D',td(2:end),...
            '_J',tj(2:end),'_A',ta(2:end),'_Sm',tsm(2:end),...
            '_BE',BE,'_CC',CC,'_lgRE',rfrac,'_mdRE',rfrac2,'_nmortTests'];
...
for s=1:length(spots)
    ...
    for t=1:np
        text(t,-2.1,num2str(morts(t)),'Rotation',45,'HorizontalAlignment','right')
    end
    if (s==11)
        text(np+2,1.5,['D=' num2str(D)]);
        text(np+2,1,['A=' num2str(Ad)]);
        text(np+2,0.5,['J=' num2str(J)]);
        text(np+2,0,['Sm=' num2str(Sm)]);
        text(np+2,-0.5,['RE=' rfrac]);
        text(np+2,-1,['enc=' tefn]);
        text(np+2,-1.5,['cmax-metab=' tcfn]);
    end
    stamp(cfile2)
...
% subplot(3,1,1)
% bar(mcon(1,:))
% set(gca,'XTick',1:np,'XTickLabel',morts); hold on;
...
bar(mlev(1,:))
ylim([0 1])
set(gca,'XTick',1:np,'XTickLabel',morts,'YTick',0.2:0.2:0.8)
ylabel('S')
