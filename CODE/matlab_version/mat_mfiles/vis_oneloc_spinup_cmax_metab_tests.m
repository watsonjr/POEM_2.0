...
figp = '/Users/cpetrik/Dropbox/Princeton/POEM_2.0/CODE/Figs/PNG/Matlab_Big_sizes/Bio_rates/';
...
for E = 1:length(encs)
    ...
    for C = 1:length(cmaxs)
        ...
        for R = 1:length(RE)
            ...
            fpath = [figp char(dp) '/'];
            cfile = char(dp);
            cfile2 = ['Dc_TrefO_cmax-metab',tcfn,'_enc',tefn,'_MFeqMP_fcrit',num2str(fcrit),'_',pref,...
                '_nmort',nmort,'_BE',BE,'_CC',CC,'_rateEQtests'];
        ...
        for s=1:length(spots)
            ...
            f2=figure(2);
            subplot(4,3,s)
            plot(1-0.25:5,log10(squeeze(fishsp(1,s,:))),'sk','MarkerFaceColor',cmap_ppt(3,:),...
                'MarkerSize',15); hold on;
            plot(1:5,log10(squeeze(fishsp(2,s,:))),'sk','MarkerFaceColor',cmap_ppt(1,:),...
                'MarkerSize',15); hold on;
            plot(1+0.25:6,log10(squeeze(fishsp(3,s,:))),'sk','MarkerFaceColor',cmap_ppt(2,:),...
                'MarkerSize',15); hold on;
            xlim([0 6])
            ylim([-2 2])
            if (s==4)
            ...
            for t=1:5
                text(t,-2.1,sreff{t},'Rotation',45,'HorizontalAlignment','right')
            end
            if (s==11)
                text(7,1,['enc=' Estr{E}]);
                text(7,0,['cmax-metab=' Cstr{C}]);
            end
            stamp(cfile2)
        ...
        save([datap 'Bio_rates/' cfile2 '.mat'],'fishsp');
