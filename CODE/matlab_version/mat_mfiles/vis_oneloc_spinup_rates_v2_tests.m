...
encs = logspace(2,4,10); %linspace(10,100,10); %
    ...
    close all
        ...
        for E = 1:length(encs)
            ...
            cfile2 = ['Dc_enc',tefn,'_cmax-metab_fcrit',num2str(fcrit),'_',D,'_',J,'_',Sm,...
                '_nmort',nmort,'_BE',BE,'_CC',CC,'_RE',rfrac,'_Enctests2'];
...
%     fishsp  = NaN*ones(4,length(spots),length(cmaxs));
%     mcon  = NaN*ones(3,length(cmaxs));
...
% subplot(3,1,1)
% bar(mcon(1,:))
% set(gca,'XTick',1:10,'XTickLabel',cmaxs); hold on;
% plot(0.5:10.5, Consumption(1,:),'--k','LineWidth',2)
% ylabel('S')
% title('Mean consumption (g yr^-^1)')
% 
...
% bar(mlev(1,:))
% ylim([0 1])
% set(gca,'XTick',1:10,'XTickLabel',cmaxs,'YTick',0.2:0.2:0.8)
% ylabel('S')
% title('Mean feeding level')
% 
% subplot(3,1,2)
