...
# load data
vec <- read.csv("Climatol_All_fish03_param_sens_v2_vecs_untrans.csv",sep=",",header = T)

#raw data
vmat <- as.matrix(vec[3:39,2:6])
rvec <- as.data.frame(t(vmat))
names(rvec) <- vec$Row[3:39]
...
pdf(paste0(fpath,"Climatol_All_fish03_pvclust_wardD_pvals_untrans_noH5.pdf"))
plot(fit2) # dendogram with p values
# add rectangles around groups highly supported by the data
pvrect(fit2, alpha=.95)
dev.off()

fit3 <- pvclust(nvec, method.hclust="complete",
                method.dist="euclidean")
pdf(paste0(fpath,"Climatol_All_fish03_pvclust_complete_pvals_untrans_noH5.pdf"))
...
## ----------------------------------- dendextend -------------------------------------------
pdf(paste0(fpath,"Climatol_All_fish03_param_sens_pairplot_untrans_noH5.pdf"))
...
# And plot:
pdf(paste0(fpath,"Climatol_All_fish03_hclust_complete_untrans_noH5.pdf"))
par(mfrow = c(1,1))
par(mar = c(3,3,3,7))
plot(dend, 
...
pdf(paste0(fpath,"Climatol_All_fish03_hclust_complete_heatmap_untrans_noH5.pdf"))
...
pdf(paste0(fpath,"Climatol_All_fish03_corr_diff_methods_untrans_noH5.pdf"))
corrplot(iris_dendlist_cor, "pie", "lower")
dev.off()


pdf(paste0(fpath,"Climatol_All_fish03_diff_methods_untrans_noH5.pdf"))
par(mfrow = c(4,2))
for(i in 1:8) {
...
## -------------- Diff methods -------------------------------
#iris_dendlist %>% dendlist(which=1:8) %>% set("branches_k_color", k=5)
#iris_dendlist %>% dendlist(which = 1) %>% set("branches_k_color", k=5)


pdf(paste0(fpath,"Climatol_All_fish03_hclust_wardD_heatmap_untrans_noH5.pdf"))
gplots::heatmap.2(as.matrix(t(nvec)), 
...
pdf(paste0(fpath,"Climatol_All_fish03_hclust_single_heatmap_untrans_noH5.pdf"))
gplots::heatmap.2(as.matrix(t(nvec)), 
...
pdf(paste0(fpath,"Climatol_All_fish03_hclust_average_heatmap_untrans_noH5.pdf"))
gplots::heatmap.2(as.matrix(t(nvec)), 
...
pdf(paste0(fpath,"Climatol_All_fish03_hclust_mcquitty_heatmap_untrans_noH5.pdf"))
gplots::heatmap.2(as.matrix(t(nvec)), 
...
pdf(paste0(fpath,"Climatol_All_fish03_hclust_median_heatmap_untrans_noH5.pdf"))
gplots::heatmap.2(as.matrix(t(nvec)), 
...
pdf(paste0(fpath,"Climatol_All_fish03_hclust_centroid_heatmap_untrans_noH5.pdf"))
gplots::heatmap.2(as.matrix(t(nvec)), 
...
pdf(paste0(fpath,"Climatol_All_fish03_hclust_wardD2_heatmap_untrans_noH5.pdf"))
gplots::heatmap.2(as.matrix(t(nvec)), 
