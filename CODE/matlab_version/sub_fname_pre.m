%%%% File naming system
function fname = sub_fname_pre(frate)
...
if (frate==0)
    fname = ['/Volumes/GFDL/NC/Matlab_new_size/',simname, '/Preindust'];
elseif (Jsel~=0.1)
    fname = ['/Volumes/GFDL/NC/Matlab_new_size/',simname, '/Preindust_', sel,'_fish',tfish(2:end),'_Juve',tJ(2:end)];
elseif (MFsel~=LPsel)
    fname = ['/Volumes/GFDL/NC/Matlab_new_size/',simname, '/Preindust_fish_F',tF(2:end),'_P',tP(2:end),'_D',tD(2:end)];
else
    fname  = ['/Volumes/GFDL/NC/Matlab_new_size/',simname, '/Preindust_', sel,'_fish',tfish(2:end)];  
end
