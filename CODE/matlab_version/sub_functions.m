...
function get_COBALT(COBALT,ID,DY,ENVR)
    ...
    ENVR.fZm(:,1) = integer(zeros(NX,1));
    ENVR.fZl(:,1) = integer(zeros(NX,1));
    ENVR.fB(:,1)  = integer(zeros(NX,1));
    ENVR.H(:,1)   = GRD('Z')(ID);
...
%%%  Encounter rates
function sub_enc(Tp,Tb,wgt,prey,tpel,tprey,pref)
    % Tp: pelagic temp
    % Tb: bottom temp
    % wgt: ind weight of size class
...
function sub_enc_pel(Tp,Tb,wgt,prey,tpel,tprey,pref)
    % Tp: pelagic temp
    % Tb: bottom temp
    % wgt: ind weight of size class
    % pred: pred biomass density,
...
%%%  Encounter rates for demersal
function sub_enc_dem(Tp,Tb,wgt,prey,tpel,tprey,pref)
    % Tp: pelagic temp
    % Tb: bottom temp
    % wgt: ind weight of size class
    % pred: pred biomass density,
...
function sub_offline_bent(enc_1,enc_2,bio_1,bio_2,B)
    con_1 = enc_1 .* bio_1;
    con_2 = enc_2 .* bio_2;
    if ((con_1 + con_2) > B)
        frac1 = con_1 ./ (con_1 + con_2);
