###! Get COBALT data
function get_COBALT!(COBALT,ID,DY,ENV_Tp,ENV_Tb,ENV_Zoo,ENV_dZ,ENV_det)
    ## Get data
    ENV_Tp[:,1] = float64(COBALT["Tp"][ID,DY])
    ENV_Tb[:,1] = COBALT["Tb"][ID,DY]
    ENV_Zoo[:,1] = COBALT["Zm"][ID,DY]
    ENV_Zoo[:,2] = COBALT["Zl"][ID,DY]
    ENV_dZ[:,1] = COBALT["dZm"][ID,DY]
    ENV_dZ[:,2] = COBALT["dZl"][ID,DY]
...
function sub_tau!(FISH::fish,i::Int,X::Int)
